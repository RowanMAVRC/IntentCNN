...
def load_flight_data(data_path: str, 
    ...
    if not os.path.exists(label_path) or label_path == "" or label_path is None:
        raise FileNotFoundError(f"Labels file '{label_path}' does not exist.")
    ...
    normalized_trajectories = normalize_single(remaining_trajectories)
    mean_removed_trajectories = mean_removed_single(normalized_trajectories)
    ...
    train_trajectories = np.array(mean_removed_trajectories)
